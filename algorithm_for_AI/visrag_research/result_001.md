...
**작성일**: 2026-05-26 KST  
**상태**: 설계 완료 / 실험 대기 (Expected Results 단계)  
...
VisRAG의 VLM generator가 검색된 다수 페이지를 처리할 때 발생하는 context window 병목 문제를 해결하기 위해, retriever와 generator 사이에 경량 cross-attention 기반 Query-Conditioned Visual Token Pruning (QCVTP) 모듈을 제안한다. 쿼리 임베딩과 각 페이지의 visual patch token 간 relevance score를 계산하여 동적 token budget 하에 상위 patch만 선택함으로써, 동일 context 예산 내에서 더 많은 페이지를 수용하고 노이즈 patch를 제거한다. SlideVQA에서 +3~5%p, DocVQA에서 +1~2%p 성능 향상이 기대되며, 추가 파라미터는 ~2M에 불과하다.
...
- **한계 #7 (Context window 제한)**: MiniCPM-V-2.6 기준 이미지당 최대 1,024 patch token이 생성되며, K=5 페이지 처리 시 5,120 token이 visual context를 점유한다.
...
1. **경량 cross-attention selector 삽입**: 기존 VisRAG 아키텍처를 최소한으로 변경하면서 retriever-generator 사이에 ~2M 파라미터의 token selector 모듈을 삽입한다.
...
3. **retriever-generator 사이에 삽입되는** 경량 cross-attention selector (RegionRAG, VimRAG는 방법론이 상이)
...
쿼리 텍스트 임베딩 e_q ∈ R^{query_dim}와 각 페이지 k의 patch token h_i ∈ R^{patch_dim}에 대해 relevance score를 계산한다:

```
q_proj = normalize(W_q · e_q)             ∈ R^{proj_dim}
v_proj_i = normalize(W_v · h_i)           ∈ R^{proj_dim}
s_i = q_proj^T · v_proj_i                 ∈ R
```

여기서 W_q ∈ R^{proj_dim × query_dim}, W_v ∈ R^{proj_dim × patch_dim}는 학습 가능한 경량 projection matrix (proj_dim=256, 총 ~2M 파라미터)이다.

L2 정규화를 적용함으로써 스케일에 무관한 안정적인 cosine-like similarity를 얻는다.
...
K=1일 때는 selector를 bypass하여 원본 token을 그대로 사용한다.
...
손실 함수:
```
L_total = L_VQA + λ · L_hinge
```

L_VQA: 정답 토큰에 대한 cross-entropy loss  
L_hinge: answer page의 평균 patch score가 noise page보다 높도록 유도하는 hinge loss

```
L_hinge = max(0, margin - mean_{i ∈ answer_page}(s_i) + mean_{j ∈ noise_page}(s_j))
```

answer page는 OCR 기반 distant supervision으로 결정 (gold answer 문자열 포함 여부).  
answer page가 식별 불가능한 샘플은 L_hinge를 skip하고 L_VQA만 사용한다.

하이퍼파라미터: λ=0.1, margin=0.5, lr=1e-4 (cosine schedule, warmup 500 steps), max_steps=2,000 (Stage 2: 10,000)
...
이미지 전처리: MiniCPM-V-2.6 기본 설정 유지 (동적 슬라이싱, 448×448).
...
| VisRAG + QCVTP (Ours) | 학습된 cross-attention selector + dynamic budget | 제안 방법 |
...
| 항목 | 설정 |
|------|------|
| Base VLM | MiniCPM-V-2.6 (bfloat16) |
| GPU | NVIDIA B200 × 2 (GPU 2, 3; 각 183 GB VRAM) — GPU 0,1은 기존 작업 점유 |
| Precision | BF16 (B200 native) |
| Batch size | Stage 1: 8 × grad_accum 2 = effective 16; Stage 3: DDP 2-GPU |
| Token budget B | 2,048 |
| N (patches/page) | 1,024 (MiniCPM-V-2.6 기준) |
| Optimizer | AdamW, weight_decay=1e-4 |
| num_workers | Stage 2: 16, Stage 3: 32 (288 vCPU 환경) |
| Checkpoint 주기 | 매 1,000 steps; best val ANLS 보존 |
...
본 연구는 VisRAG의 context window 병목 문제를 해결하기 위한 QCVTP (Query-Conditioned Visual Token Pruning) 방법을 제안한다. 핵심 아이디어는 retriever-generator 사이에 경량 cross-attention selector를 삽입하여, 쿼리 관련성 기준으로 visual patch token을 동적으로 pruning하는 것이다. 이를 통해 동일 context 예산 내에서 더 많은 페이지를 처리하고 (K=3→5~7), 노이즈 patch를 제거하여 답변 품질을 향상시키는 것을 목표로 한다.

Novelty 검증 결과 QG-VTC, AVIR, RegionRAG 등 유사 연구와 차별화된다. 특히 VisRAG retrieval pipeline + 멀티페이지 dynamic budget + cross-attention selector의 세 요소 결합은 기존 문헌에서 발견되지 않았다.
